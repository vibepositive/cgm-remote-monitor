...
function init (client, d3, $) {
  ...
  // Enable two-finger horizontal pinch zoom on the main chart, including
  // iOS Home Screen web apps. This only changes the visible time window.
  var pinchZoom = {
    active: false,
    startDistance: 0,
    startRangeMS: 0,
  ...
  function clampPinchRange (rangeMS) {
    var minRangeMS = times.mins(30).msecs;
    var dataExtent = client.dataExtent();
    var availableRangeMS = Math.max(minRangeMS, dataExtent[1].getTime() - dataExtent[0].getTime());
    var maxRangeMS = Math.min(times.hours(24).msecs, availableRangeMS);
    return Math.max(minRangeMS, Math.min(maxRangeMS, rangeMS));
  }

  function beginPinchZoom (event) {
    if (!chart.theBrush || event.touches.length !== 2) return;

    var rect = document.getElementById('chartContainer').getBoundingClientRect();
    var centerX = ((event.touches[0].clientX + event.touches[1].clientX) / 2) - rect.left;

    pinchZoom.active = true;
    pinchZoom.startDistance = touchDistance(event.touches);
    pinchZoom.startRangeMS = client.focusRangeMS;
    pinchZoom.centerTime = chart.xScale.invert(centerX);
    event.preventDefault();
  }

  function movePinchZoom (event) {
    if (!pinchZoom.active || event.touches.length !== 2 || !chart.theBrush) return;

    var distance = touchDistance(event.touches);
    if (!distance || !pinchZoom.startDistance) return;

    // Spreading fingers shows less time; pinching together shows more time.
    var newRangeMS = clampPinchRange(
      pinchZoom.startRangeMS * pinchZoom.startDistance / distance
    );

    var center = pinchZoom.centerTime.getTime();
    var from = center - newRangeMS / 2;
    var to = center + newRangeMS / 2;
    var extent = client.dataExtent();
    var min = extent[0].getTime();
    var max = extent[1].getTime();
    ...
    client.focusRangeMS = newRangeMS;
    chart.theBrush.call(chart.brush.move, [
      chart.xScale2(new Date(from)),
      chart.xScale2(new Date(to))
    ]);
  ...
  function endPinchZoom (event) {
    if (pinchZoom.active && event.touches.length < 2) {
      ...
      pinchZoom.centerTime = null;
    }
  }

  // One-finger horizontal pan. We wait until horizontal movement clearly
  // exceeds vertical movement before taking control, so ordinary page
  // scrolling still works naturally on phones.
  var touchPan = {
    active: false,
    locked: false,
    startX: 0,
  ...
  function beginTouchPan (event) {
    if (!chart.theBrush || event.touches.length !== 1) {
      touchPan.active = false;
      touchPan.locked = false;
      return;
    }

    var selection = d3.brushSelection(chart.theBrush.node());
    if (!selection) return;

    touchPan.active = true;
    touchPan.locked = false;
    touchPan.startX = event.touches[0].clientX;
    touchPan.startY = event.touches[0].clientY;
    touchPan.from = chart.xScale2.invert(selection[0]).getTime();
    touchPan.to = chart.xScale2.invert(selection[1]).getTime();
  }

  function moveTouchPan (event) {
    if (!touchPan.active || event.touches.length !== 1 || !chart.theBrush) return;

    var dx = event.touches[0].clientX - touchPan.startX;
    var dy = event.touches[0].clientY - touchPan.startY;

    if (!touchPan.locked) {
      // Small movements are ambiguous. Once the gesture is clearly
      // horizontal, lock it to chart navigation for the rest of the swipe.
      if (Math.abs(dx) < 8 && Math.abs(dy) < 8) return;
      if (Math.abs(dy) >= Math.abs(dx)) {
        touchPan.active = false;
        return;
      }
      touchPan.locked = true;
    }

    var width = chart.xScale2.range()[1] - chart.xScale2.range()[0];
    if (!width) return;

    var rangeMS = touchPan.to - touchPan.from;
    var shiftMS = -dx / width * rangeMS;
    var from = touchPan.from + shiftMS;
    var to = touchPan.to + shiftMS;
    var extent = client.dataExtent();
    var min = extent[0].getTime();
    var max = extent[1].getTime();
  ...
  var chartTouchTarget = document.getElementById('chartContainer');
  if (chartTouchTarget && !chartTouchTarget.__nightscoutPinchZoom) {
    chartTouchTarget.__nightscoutPinchZoom = true;

    // Safari/iOS can route a two-finger pinch through gesture events rather
    // than ordinary touchmove events, especially in a Home Screen web app.
    // Support both paths.
    var safariGesture = {
      active: false,
      startRangeMS: 0,
    ...
    function beginSafariGesture (event) {
      if (!chart.theBrush) return;
      var rect = chartTouchTarget.getBoundingClientRect();
      var centerX = (typeof event.clientX === 'number' ? event.clientX : rect.left + rect.width / 2) - rect.left;
      safariGesture.active = true;
      safariGesture.startRangeMS = client.focusRangeMS;
      safariGesture.centerTime = chart.xScale.invert(centerX);
      event.preventDefault();
    }

    function moveSafariGesture (event) {
      if (!safariGesture.active || !chart.theBrush || !event.scale) return;

      var newRangeMS = clampPinchRange(safariGesture.startRangeMS / event.scale);
      var center = safariGesture.centerTime.getTime();
      var from = center - newRangeMS / 2;
      var to = center + newRangeMS / 2;
      var extent = client.dataExtent();
      var min = extent[0].getTime();
    ...
    function endSafariGesture (event) {
      safariGesture.active = false;
      safariGesture.centerTime = null;
      if (event) event.preventDefault();
    }

    chartTouchTarget.addEventListener('touchstart', beginPinchZoom, { passive: false });
    chartTouchTarget.addEventListener('touchstart', beginTouchPan, { passive: false });
    chartTouchTarget.addEventListener('touchmove', movePinchZoom, { passive: false });
    chartTouchTarget.addEventListener('touchmove', moveTouchPan, { passive: false });
    chartTouchTarget.addEventListener('touchend', endPinchZoom, { passive: false });
    chartTouchTarget.addEventListener('touchend', endTouchPan, { passive: false });
    chartTouchTarget.addEventListener('touchcancel', endPinchZoom, { passive: false });
    chartTouchTarget.addEventListener('touchcancel', endTouchPan, { passive: false });
    chartTouchTarget.addEventListener('gesturestart', beginSafariGesture, { passive: false });
    chartTouchTarget.addEventListener('gesturechange', moveSafariGesture, { passive: false });
    chartTouchTarget.addEventListener('gestureend', endSafariGesture, { passive: false });
  }
