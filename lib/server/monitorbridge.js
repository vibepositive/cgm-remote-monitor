...
module.exports = function monitorBridge(env) {
  ...
  const port = env.PORT || process.env.PORT || 1337;
  const base = `http://127.0.0.1:${port}`;

  router.get('/health', (req, res) => {
    res.json({ ok: true, service: 'nightscout-monitor', configured: Boolean(key && token) });
  });

  router.get('/24h', async (req, res) => {
    try {
      const supplied = req.get('x-monitor-key') || req.query.key;
      if (!key || !token) return res.status(503).json({ ok: false, error: 'monitor not configured' });
      ...
      async function ns(path) {
        ...
        let response;
        try {
          response = await fetch(`${base}${path}${separator}token=${encodeURIComponent(token)}`, {
            headers: {
          ...
          throw new Error(`Fetch failed for ${path}: ${err.message}`);
        }
        if (!response.ok) {
          let body = '';
          try { body = await response.text(); } catch (_) {}
          throw new Error(`Nightscout ${path} returned ${response.status}${body ? `: ${body.slice(0, 300)}` : ''}`);
        }
        try {
      ...
      const [entriesRaw, treatmentsRaw, deviceRaw, profilesRaw] = await Promise.all([
        ns('/api/v1/entries.json?count=500'),
        ns('/api/v1/treatments.json?count=1500'),
        ns('/api/v1/devicestatus.json?count=500'),
      ...
      res.json({
        ...
        profile: Array.isArray(profilesRaw) ? (profilesRaw[0] || null) : null
      });
    } catch (err) {
      console.error('Nightscout monitor /24h failed:', err && err.stack ? err.stack : err);
      return res.status(502).json({
        ok: false,
        error: 'monitor data fetch failed',
