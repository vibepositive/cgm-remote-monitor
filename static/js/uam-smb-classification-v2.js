...
(function () {
  if (window.__nightscoutUamSmbClassificationV4Loaded) return;
  window.__nightscoutUamSmbClassificationV4Loaded = true;
  ...
  function hideNativeInsulinArtifacts(container) {
    Array.prototype.slice.call(container.querySelectorAll('*')).forEach(function (node) {
      if (node.closest && node.closest('.uam-smb-v2-overlay')) return;
      var d = node.__data__;
      if (!d || !(n(d.insulin) > 0)) return;
      var target = node;
      if (node.closest) {
        var group = node.closest('g');
        if (group && !(group.closest && group.closest('.uam-smb-v2-overlay'))) target = group;
      }
      target.setAttribute('data-classified-insulin-native', '1');
    });
  }
  ...
  function ensureUi(scale) {
    ...
    if (!legend) {
      ...
      legend.innerHTML = '<div style="font-size:10px;opacity:.72;margin-bottom:5px;letter-spacing:.04em">INSULIN EVENTS</div><div style="display:flex;gap:10px;flex-wrap:wrap"><span style="color:' + COLORS.manual + '">▲ Manual</span><span style="color:' + COLORS.smb + '">◆ SMB</span><span style="color:' + COLORS.uam + '">● UAM SMB</span><span style="color:' + COLORS.capped + '">◎ UAM cap</span></div>';
  ...
  function draw(scale, data) {
    ...
    hideNativeInsulinArtifacts(scale.container);
    ...
    var laneY = 58;
    var label = svgEl('text', { x: 8, y: laneY - 19, fill: 'rgba(255,255,255,.62)', 'font-size': 10, 'font-weight': 700 });
    label.textContent = 'INSULIN EVENTS';
    ui.overlay.appendChild(label);

    var line = svgEl('line', { x1: 0, y1: laneY + 14, x2: scale.width, y2: laneY + 14, stroke: 'rgba(255,255,255,.10)', 'stroke-width': 1 });
  ...
  function scheduleRender(mutations) {
    if (Array.isArray(mutations)) {
      var onlyOurs = mutations.length && mutations.every(function (m) {
        var t = m.target;
        return t && t.closest && (t.closest('.uam-smb-v2-overlay') || t.closest('.uam-smb-v2-tooltip') || t.closest('.uam-smb-v2-legend'));
      });
      if (onlyOurs) return;
    }
